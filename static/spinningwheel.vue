...
export default {
  ...
  methods: {
    ...
    async onStopWheelSpin(winningEntry) {
      this.$store.commit('setWheelBusy', false);
      this.$emit('wheel-stopped', winningEntry);
      this.$store.commit('setWheelSpinCount', this.$store.getters.wheelSpinCount + 1)
      if (this.$store.getters.wheelSpinCount > this.$store.getters.wheelReloadMovies - 1) {
